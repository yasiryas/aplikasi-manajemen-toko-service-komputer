...
export function swalConfirm(title, text, confirmLabel = 'Ya, lanjutkan', dangerType = 'warning') {
    return Swal.fire({
        title,
        text,
        icon: dangerType,
        ...
        confirmButtonColor: dangerType === 'error' ? '#dc2626' : '#4f46e5',
...
async function openOrderDetail(orderId) {
    ...
    body.innerHTML = '<p class="py-6 text-center text-sm text-slate-400">Memuat&hellip;</p>';

    try {
        const { order } = await ajax.get(`/service-orders/${orderId}`);
        body.innerHTML = renderOrderDetail(order);
    } catch {
        body.innerHTML = '<p class="py-6 text-center text-sm text-rose-600">Gagal memuat detail tiket.</p>';
    }
}
...
function renderOrderDetail(order) {
    ...
    const logs = (order.logs ?? [])
        ...
        .join('') || '<li class="text-sm text-slate-400">Belum ada catatan.</li>';

    const notifications = (order.notification_logs ?? [])
        .map(
            (n) => `<p class="text-xs text-slate-500 flex items-center gap-1.5">
...
const orderForm = {
    ...
    bindEvents() {
        ...
        search.addEventListener('input', debounce(() => this.searchCustomers(search.value), 350));
        document.getElementById('order-customer-results').addEventListener('click', (e) => {
            const li = e.target.closest('[data-customer-id]');
            if (li) this.selectCustomer(Number(li.dataset.customerId), li.dataset.customerName);
        });

        document.getElementById('order-add-device-btn')?.addEventListener('click', () => {
            document.getElementById('order-new-device').classList.remove('hidden');
    ...
    async searchCustomers(q) {
        ...
        if (!customers.length) {
            results.innerHTML = '<li class="px-3 py-2 text-slate-400">Tidak ditemukan.</li>';
        } else {
            customers.forEach((c) => {
    ...
    async selectCustomer(id, name) {
        ...
        if (devices.length) {
            document.getElementById('order-device-container').classList.remove('hidden');
            document.getElementById('order-new-device').classList.add('hidden');
            fillSelect(document.getElementById('order-device-id'), devices.map((d) => ({ value: d.id, label: `${d.jenis.toUpperCase()} ${d.merk} ${d.model ?? ''} — ${d.keluhan.slice(0, 40)}` })), 'Pilih perangkat…');
        } else {
            document.getElementById('order-device-container').classList.remove('hidden');
            document.getElementById('order-new-device').classList.remove('hidden');
            fillSelect(document.getElementById('order-device-id'), [], 'Belum ada perangkat');
        }
    },

    async loadDevices(customerId) {
        const { devices } = await ajax.get(`/api/customers/${customerId}/devices`);
        return devices;
    },
    ...
    async submit(e) {
        ...
        try {
            ...
            if (!deviceId && customerId && !document.getElementById('order-new-device').classList.contains('hidden')) {
                const customer_id = Number(document.getElementById('order-new-device-customer').value);
                const { device } = await ajax.post('/devices', {
                    ...
                    keluhan: document.getElementById('order-device-keluhan').value,
                });
                deviceId = device.id;
            }
            ...
            const payload = {
                ...
                teknisi_id: document.getElementById('order-tech').value ? Number(document.getElementById('order-tech').value) : null,
                estimasi_biaya: document.getElementById('order-estimate').value ? Number(document.getElementById('order-estimate').value) : null,
...
document.addEventListener('change', async (event) => {
    ...
    try {
        await ajax.patch(`/service-orders/${orderId}/status`, { status: select.value });
...
window.RepairStation = {
    ...
    reSendNotification: async (orderId) => {
        try {
            const { message } = await ajax.post(`/service-orders/${orderId}/notify`);
            toast(message);
        } catch {
    ...
    deleteOrder: async (orderId) => {
        const { isConfirmed } = await swalConfirm('Hapus tiket ini?', 'Data tiket akan dihapus permanen.', 'Ya, hapus', 'error');
